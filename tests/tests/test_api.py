...
class TestPageChooserApi(TestCase):
    ...
    def test_correct_digest(self):
        digest = digest_for_source(
            "local", "child_of=root&fields=parent%2Cchildren&limit=20&offset=0"
        )
        response = self.client.get(
...
class TestModelsApi(TestCase):
    ...
    def test_model_chooser_response(self):
        response = self.client.get(
            f"/wagtail-transfer/api/chooser/models/{self.get_parameters()}"
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_model_object_chooser(self):
        response = self.client.get(
            f"/wagtail-transfer/api/chooser/models/tests.category/{self.get_parameters()}"
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_model_object_previous_and_next_pagination(self):
        ...
        response = self.client.get(
            f'/wagtail-transfer/api/chooser/models/tests.category/{self.get_parameters("page=2")}'
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_model_object_previous_pagination(self):
        ...
        response = self.client.get(
            f'/wagtail-transfer/api/chooser/models/tests.category/{self.get_parameters("page=3")}'
        )
        self.assertEqual(response.status_code, 200)
...
class TestPagesApi(TestCase):
    ...
    def get(self, page_id, recursive=True):
        ...
        return self.client.get(
            "/wagtail-transfer/api/pages/%d/?digest=%s&recursive=%s"
            % (page_id, digest, str(recursive).lower())
        )
    ...
    def test_pages_api(self):
        ...
        self.assertIn(
            ["wagtailcore.page", 2, "22222222-2222-2222-2222-222222222222"], mappings
        )
        self.assertIn(
            ["tests.advert", 1, "adadadad-1111-1111-1111-111111111111"], mappings
        )
    ...
    def test_parental_keys(self):
        page = SectionedPage(
            title="How to make a cake", intro="Here is how to make a cake."
        )
        page.sections.create(
            title="Create the universe", body="First, create the universe"
        )
        ...
        matching_uids = [
            uid
            for model_name, pk, uid in data["mappings"]
    ...
    def test_rich_text_with_page_link(self):
        page = PageWithRichText(
            title="You won't believe how rich this cake was!",
            body='<p>But I have a <a id="1" linktype="page">link</a></p>',
        )
    ...
    def test_rich_text_with_dead_page_link(self):
        page = PageWithRichText(
            title="You won't believe how rich this cake was!",
            body='<p>But I have a <a id="999" linktype="page">link</a></p>',
        )
    ...
    def test_null_rich_text(self):
        ...
        self.assertTrue(any(obj["pk"] == page.pk for obj in data["objects"]))
    ...
    def test_rich_text_with_image_embed(self):
        ...
        body = (
            '<p>Here is an image</p><embed embedtype="image" id="%d" alt="A wagtail" format="left" />'
            % image.pk
        )
    ...
    def test_streamfield_with_rich_text(self):
        ...
        page = PageWithStreamField(
            title="My streamfield rich text block has a link",
            body=json.dumps(
                [
                    {
                        "type": "rich_text",
    ...
    def test_parental_many_to_many(self):
        ...
        self.assertIn(
            ["tests.advert", 2, "adadadad-2222-2222-2222-222222222222"],
            data["mappings"],
        )
        self.assertIn(
            ["tests.advert", 3, "adadadad-3333-3333-3333-333333333333"],
            data["mappings"],
        )
...
class TestObjectsApi(TestCase):
    ...
    def test_incorrect_digest(self):
        request_body = json.dumps({"tests.advert": [1]})

        response = self.client.post(
            "/wagtail-transfer/api/objects/?digest=12345678",
            request_body,
    ...
    def get(self, request_body):
        ...
        return self.client.post(
            "/wagtail-transfer/api/objects/?digest=%s" % digest,
            request_json,
            content_type="application/json",
        )

    def test_objects_api(self):
        response = self.get({"tests.advert": [1]})
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
        ...
        self.assertEqual(
            data["objects"][0]["fields"]["slogan"], "put a tiger in your tank"
        )
        self.assertEqual(
            data["objects"][0]["fields"]["run_until"], "2020-12-23T21:00:00Z"
        )
        self.assertEqual(data["objects"][0]["fields"]["run_from"], None)

        self.assertEqual(
            data["mappings"],
            [["tests.advert", 1, "adadadad-1111-1111-1111-111111111111"]],
        )

    def test_objects_api_with_tree_model(self):
        root_collection = Collection.objects.get()
        collection = root_collection.add_child(
            instance=Collection(name="Test collection")
        )
        ...
        response = self.get({"wagtailcore.collection": [collection.id]})
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
    ...
    def test_many_to_many(self):
        ...
        response = self.get({"tests.modelwithmanytomany": [1]})
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
    ...
    def test_model_with_field_lookup(self):
        response = self.get({"tests.category": [1]})
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
    ...
    def test_model_with_multi_table_inheritance(self):
        ...
        response = self.get({"tests.advert": [long_ad.pk]})
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
    ...
    def test_model_with_tags(self):
        ...
        response = self.get({"tests.advert": [ad.pk]})
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
    ...
    def test_image(self):
        ...
        obj = data["objects"][0]
        self.assertEqual(
            obj["fields"]["file"]["download_url"],
            "http://media.example.com/media/original_images/wagtail.jpg",
        )
        self.assertEqual(obj["fields"]["file"]["size"], 1160)
        self.assertEqual(
            obj["fields"]["file"]["hash"], "45c5db99aea04378498883b008ee07528f5ae416"
        )
    ...
    def test_image_with_local_media_url(self):
        ...
        self.assertEqual(
            obj["fields"]["file"]["download_url"],
            "http://example.com/media/original_images/wagtail.jpg",
        )
        self.assertEqual(obj["fields"]["file"]["size"], 1160)
        self.assertEqual(
            obj["fields"]["file"]["hash"], "45c5db99aea04378498883b008ee07528f5ae416"
    ...
    def test_document(self):
        with open(os.path.join(FIXTURES_DIR, "document.txt"), "rb") as f:
            document = Document.objects.create(
                title="Test document", file=File(f, name="document.txt")
            )

        response = self.get({"wagtaildocs.document": [document.pk]})
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
        ...
        self.assertEqual(
            obj["fields"]["file"]["download_url"],
            "http://media.example.com/media/documents/document.txt",
        )
        self.assertEqual(obj["fields"]["file"]["size"], 33)
        self.assertEqual(
            obj["fields"]["file"]["hash"], "9b90daf19b6e1e8a4852c64f9ea7fec5bcc5f7fb"
        )

    def test_custom_model_with_file_field(self):
        with open(os.path.join(FIXTURES_DIR, "wagtail.jpg"), "rb") as f:
            avatar = Avatar.objects.create(image=ImageFile(f, name="wagtail.jpg"))
...
class TestChooserProxyApi(TestCase):
    ...
    def test_with_unknown_source(self, get):
        ...
        response = self.client.get(
            "/admin/wagtail-transfer/api/chooser-proxy/production/foo?bar=baz",
            HTTP_ACCEPT="application/json",
        )
