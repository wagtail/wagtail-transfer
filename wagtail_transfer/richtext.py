...
class RichTextReferenceHandler:
    ...
    def update_tag_id(self, match, destination_ids_by_source):
        ...
        try:
            ...
            new_tag_string = (
                match.group(0)[: (match.start(1) - tag_body_offset)]
                + new_tag_body
                + match.group(0)[(match.end(1) - tag_body_offset) :]
            )
    ...
    def update_ids(self, html, destination_ids_by_source):
        ...
        if html is None:
            ...
            return self.tag_matcher.sub(
                partial(
                    self.update_tag_id,
                    destination_ids_by_source=destination_ids_by_source,
                ),
                html,
            )
